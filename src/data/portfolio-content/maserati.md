...
부트캠프에서 Three.js를 처음 접하며,

>**이 기술을 실제 서비스에 어떻게 적용할 수 있을지** 궁금증을 갖게 되었습니다.


평소 자동차에 관심을 가지던 것을 바탕으로

차량 쇼룸을 주제로 프로젝트를 기획했고,

>기존 마세라티 공식 홈페이지를 살펴보았습니다.


공식 홈페이지에서는 슬라이드 방식으로 차량을 커스텀할 수 있었지만

**사용자가 직접 차량을 360도 회전, 확대와 축소**를 하면서

>**인터랙티브한 경험**을 할 수 있다면 좋을 것 같다고 생각했습니다.


따라서 고디테일 GLB모델을 활용해

차량 외부 컬러는 물론, 캘리퍼 컬러까지 취향대로 커스텀할 수 있으며,

>차량 외부에서 내부까지 자연스럽게 탐색할 수 있도록 합니다.

특히 스크롤 기반 카메라 이동을 통해

마치 운전석에 앉아 있는 듯한 **1인칭 시점의 경험**을 제공해

>실제 차량 시승과 유사한 몰입감을 전달하는 것을 목표로 했습니다.

더 나아가 React Three Fiber로 가상의 큰 창문을 구성하고,

야경, 도심, 숲 속, 단풍길 등 다양한 배경 속 마세라티를 연출했습니다.

이를 통해 단순한 차량 전시를 넘어,

**이 상황에서 마세라티 차주가 되면 어떤 느낌일지 간접적으로 경험 할 수 있는 쇼룸**
...
- **React**
차량 컬러, 옵션 변경 등 버튼 기반 커스텀 UI를 구현하는 데 있어

> React의 컴포넌트 구조와 props 흐름이 가장 적합하다고 판단.
...
순수 Three.js는 카메라, 조명, 씬 설정 등에서 중복 코드가 많아질 수 있는 구조

React Three Fiber는 이를 컴포넌트 단위로 추상화해 필요한 요소를 선언적으로
조합하는 방식이 가능

### 스타일
- **Tailwind CSS**

본 프로젝트는 다수의 페이지를 가지는 서비스가 아닌

단일 3D 쇼룸 중심의 UI 구조였기 때문에 Tailwind CSS만으로도 충분하다고 판단


## trouble

1. 과도한 Props Drilling 문제
...
2. OrbitControls 기반 잦은 리렌더로 인한 성능 이슈
...
3. glb 파일 로드 속도 저하 및 파일 mesh 손상
